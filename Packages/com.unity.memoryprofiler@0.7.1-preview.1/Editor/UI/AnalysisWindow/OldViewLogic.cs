...
namespace Unity.MemoryProfiler.Editor.UI
{
    internal class OldViewLogic : IViewPaneEventListener
    {
...
#endif
        public void GetCellString(Database.Table m_TableDisplay, FormattingOptions format, long row, long col, out string cellStr)
        {
            ...
            if (column != null)
            {
                ...
                var formatter = format.GetFormatter(metaColumn.FormatName);

                if (tablerows == column.GetRowCount())
                    rowStrVal = column.GetRowValueString(row, format.GetFormatter(metaColumn.FormatName));

                var idx = rowStrVal.IndexOf("\n", StringComparison.InvariantCultureIgnoreCase);
                string tooltip = null;
                if (idx != -1)
                {
                    tooltip = rowStrVal;
        ...
        public void SaveMemoryDataToFile(Database.Table table)
        {
            var format = new FormattingOptions();
            var rowCount = table.GetRowCount();
            var meta = table.GetMetaData();
            int colCount = meta.GetColumnCount();
            var saveFilePath = UIState.CurrentMode.FullName + ".csv";
            var saveTextureFilePath = UIState.CurrentMode.FullName + ".Texture2D.csv";
            var saveRTFilePath = UIState.CurrentMode.FullName + ".RT.csv";
            var saveTextureArrayFilePath = UIState.CurrentMode.FullName + ".Texture2DArray.csv";
            if (File.Exists(saveFilePath))
            {//²»ÖØ¸´±£´æ
                return;
            }
            Debug.Log($"--------{rowCount}=>{UIState.CurrentMode.FullName}");
            var logStr = new StringBuilder();
            var logTextureStr = new StringBuilder();
            var logRTStr = new StringBuilder();
            var logTextureArrayStr = new StringBuilder();
            if (rowCount > 0)
            {
                for (var i = 0; i < rowCount; i++)
                {
                    bool savable_Texture = false;
                    bool savable_RT = false;
                    bool savable_TextureArray = false;

                    string rowStr = "";
                    for (long k = 0; k < colCount; ++k)
                    {
                        string cellStr;
                        GetCellString(table, format, i, k, out cellStr);
                        if (k == 3)
                        {
                            if (cellStr == "Texture2D")
                            {
                                savable_Texture = true;
                            }
                            if (cellStr == "RenderTexture")
                            {
                                savable_RT = true;
                            }
                            if (cellStr == "Texture2DArray")
                            {
                                savable_TextureArray = true;
                            }
                        }
                        rowStr +=cellStr + ",";
                    }
     
                    logStr.Append(rowStr).Append("\n");
                    if (savable_Texture)
                    {
                        logTextureStr.Append(rowStr).Append("\n");
                    }
                    if(savable_RT)
                    {
                        logRTStr.Append(rowStr).Append("\n");
                    }
                    if(savable_TextureArray)
                    {
                        logTextureArrayStr.Append(rowStr).Append("\n");
                    }
                    if (logStr.Length > 50000)
                    {
                        var str = logStr.ToString();
                        File.AppendAllTextAsync(saveFilePath, str).Wait();
                        logStr.Clear();
                    }
                    
                    if(logTextureStr.Length > 50000)
                    {
                        File.AppendAllTextAsync(saveTextureFilePath, logTextureStr.ToString()).Wait();
                        logTextureStr.Clear();
                    }
                    if(logRTStr.Length > 50000)
                    {
                        File.AppendAllTextAsync(saveRTFilePath, logRTStr.ToString()).Wait();
                        logRTStr.Clear();
                    }

                    if (logTextureArrayStr.Length > 50000)
                    {
                        File.AppendAllTextAsync(saveTextureArrayFilePath, logTextureArrayStr.ToString()).Wait();
                        logTextureArrayStr.Clear();
                    }
                }
            }
            File.AppendAllTextAsync(saveTextureFilePath, logTextureStr.ToString()).Wait();
            File.AppendAllTextAsync(saveRTFilePath, logRTStr.ToString()).Wait();
            File.AppendAllTextAsync(saveTextureArrayFilePath, logTextureArrayStr.ToString()).Wait();
            File.AppendAllTextAsync(saveFilePath, logStr.ToString()).Wait();
            //Debug.Log(logStr);
        }
        void OpenTable(Database.TableReference tableRef, Database.Table table)
        {
            SaveMemoryDataToFile(table);
            UIState.TransitModeToOwningTable(table);
            var pane = new UI.SpreadsheetPane(m_UIStateHolder, this);
